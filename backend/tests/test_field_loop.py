...
from app import field_routes, taxon_routes
...
def test_taxon_photo_discovery_and_idempotent_remote_photo_sync(tmp_path, monkeypatch) -> None:
    ...
    def fake_inat_get(url, **kwargs):
        taxon_calls.append((url, kwargs))
        return {
            "results": [{
                "id": 12727,
    ...
    monkeypatch.setattr(taxon_routes, "_inat_get", fake_inat_get)
    ...
    assert taxon_calls[0][0].endswith("/v1/taxa")
    assert taxon_calls[0][1]["params"] == {
        "q": "American Robin",
        "per_page": 10,
        "is_active": "true",
    }
    ...
    assert taxon_calls[1][0].endswith("/v1/taxa/12727")
        ...
        with engine.begin() as connection:
            connection.execute(
                text(
                    """
                    UPDATE encounters
                    SET inat_observation_id = 1987654, inat_sync_status = 'synced'
                    WHERE encounter_id = :encounter_id
                    """
                ),
                {"encounter_id": encounter_id},
            )
        return {"encounter_id": encounter_id, "inat_observation_id": 1987654, "status": "synced"}
    ...
    def fake_http_post(url, **kwargs):
        ...
        assert kwargs["data"]["observation_photo[observation_id]"] == (
            "53411fc2-bdf0-434e-afce-4dac33970173"
        )
        assert "file" in kwargs["files"]
        return FakeResponse({"id": 246210022})

    monkeypatch.setattr(field_routes.httpx, "get", fake_http_get)
    monkeypatch.setattr(field_routes.httpx, "post", fake_http_post)
    ...
    assert get_calls[0][0].endswith("/v2/observations/1987654")
    assert get_calls[0][1]["params"] == {"fields": "id,uuid"}
    assert len(post_calls) == 1
    assert post_calls[0][0].endswith("/v2/observation_photos")
