...
def test_taxon_photo_discovery_and_idempotent_remote_photo_sync(tmp_path, monkeypatch) -> None:
    ...
    search_route = respx.get(
        ...
        params={"q": "American Robin", "per_page": 10, "is_active": "true"},
    ).mock(
    ...
    created = client.post(
        ...
        headers=headers,
        data={"metadata": metadata, "caption": "Robin evidence", "sync_inaturalist": "false"},
    ...
    # Same taxon, same Region: progress stays one discovery and no second points award.
    second = client.post(
        "/api/field/encounters",
        headers=headers,
        data={
    ...
    # Exercise the new media sync adapter without creating a real remote observation.
    ...
    uuid_route = respx.get(
        "https://api.inaturalist.org/v2/observations/987654",
        params={"fields": "id,uuid"},
    ).mock(
        return_value=Response(
            200,
            json={"results": [{"id": 987654, "uuid": "53411fc2-bdf0-434e-afce-4dac33970173"}]},
        )
    )
    ...
    # Retry is safe: UUID is cached and already-synced media are not uploaded twice.
